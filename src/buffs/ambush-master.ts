...
import { EnhancedDamageBuff } from "../interface/buff";
import { Player } from "../interface/player";
import { BuffCalcType, BuffType } from "../const/buff-type";

export class AmbushMasterBuff extends EnhancedDamageBuff {
	public name = "재빠른 기습";
	...
	initialize(): void {
		this.player.history.debug(`[${this.name}:${this.level}] 버프가 [${this.player.name}] 플레이어에게 초기화 됐습니다. 이벤트: [${GameEvents.PLAYER_TURN_END}]`);
	...
	deinitialize(): void {
		...
		this.player.delete(GameEvents.PLAYER_TURN_END, this.onTurnEnd);
